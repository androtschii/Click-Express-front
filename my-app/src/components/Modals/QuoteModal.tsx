...
interface CommonProps {
  ...
  focusRed: (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) => void;
  blurReset: (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) => void;
  isRu: boolean;
  isDark: boolean;
}

// ── Step indicator ── (outside QuoteModal so identity is stable across re-renders)
const QuoteStepIndicator: React.FC<{ step: 1 | 2 | 3; isDark: boolean; STEP_LABELS: string[] }> = ({ step, isDark, STEP_LABELS }) => (
...
const QuoteStep1: React.FC<CommonProps & { CARGO_TYPES: string[]; LOAD_TYPES: string[] }> = ({
  form, set, inputStyle, labelStyle, focusRed, blurReset, isRu, isDark, CARGO_TYPES, LOAD_TYPES,
}) => (
...
// ── Step 2: Route & Dates ──
type Estimate = { price: number; miles: number; days: number } | null;

const QuoteStep2: React.FC<CommonProps & { estimate: Estimate }> = ({
  form, set, inputStyle, labelStyle, focusRed, blurReset, isRu, isDark, estimate,
}) => (
  <div style={{ paddingTop: 24 }}>
...
const QuoteStep3: React.FC<CommonProps> = ({
  form, set, inputStyle, labelStyle, focusRed, blurReset, isRu, isDark,
}) => (
  <div style={{ paddingTop: 24 }}>
...
export const QuoteModal: React.FC<QuoteModalProps> = ({ onClose, theme: themeProp }) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/lead`, {
        ...
        body: JSON.stringify({
          ...
          weight: form.weight ? parseFloat(form.weight.replace(/,/g, "")) : null,
  ...
  const stepProps: CommonProps = { form, set, inputStyle, labelStyle, focusRed, blurReset, isRu, isDark };
        ...
        >
          {sent ? (
            <div style={{ textAlign: "center", padding: "60px 40px" }}>
              ...
              <h3 style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: 28, color: isDark ? "#fff" : "#1a1a1a", textTransform: "uppercase", margin: 0, marginBottom: 12 }}>{isRu ? "ЗАЯВКА ОТПРАВЛЕНА!" : "REQUEST SENT!"}</h3>
            ...
            <>
              ...
              {/* Step indicator */}
              <QuoteStepIndicator step={step} isDark={isDark} STEP_LABELS={STEP_LABELS} />
              ...
              <div style={{ padding: "0 36px", overflow: "hidden", flex: 1 }}>
                ...
                >
                  {step === 1 && <QuoteStep1 {...stepProps} CARGO_TYPES={CARGO_TYPES} LOAD_TYPES={LOAD_TYPES} />}
                  {step === 2 && <QuoteStep2 {...stepProps} estimate={estimate} />}
                  {step === 3 && <QuoteStep3 {...stepProps} />}
                </div>
              </div>
