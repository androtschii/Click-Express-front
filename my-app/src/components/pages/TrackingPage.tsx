...
const STATUS_TO_STEP: Record<string, number> = {
  ...
  "assigned": 2, "driver assigned": 2,
  "picked up": 3, "pickedup": 3,
  "in transit": 4, "intransit": 4, "transit": 4,
  "delivered": 5,
};
    ...
    <div style={{ position: "relative", padding: "32px 0 16px" }}>
      ...
      <div style={{ position: "relative", height: 6, borderRadius: 99, background: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)", overflow: "visible" }}>
        ...
        {progress > 0 && progress < 100 && (
          <div style={{ position: "absolute", top: "50%", left: `${progress}%`, transform: "translate(-50%,-50%)", fontSize: 20, zIndex: 3, filter: "drop-shadow(0 2px 6px rgba(204,0,0,0.6))", transition: "left 1.2s cubic-bezier(0.4,0,0.2,1)" }}>🚛</div>
        )}
        {progress >= 100 && (
...
export const TrackingPage: React.FC<TrackingPageProps> = ({
  ...
  useEffect(() => {
    if (!orderId || !session) { setHistoryLoaded(true); return; }
    fetch(`${apiBase}/order/${orderId}/tracking`, {
      headers: { Authorization: `Bearer ${session.token}` },
    })
      .then(r => r.ok ? r.json() : Promise.reject())
      .then((data: HistoryEntry[]) => {
        setHistory(data);
        setCancelled(data.some(e => e.status.toLowerCase() === "cancelled"));
      })
      .catch(() => {})
      .finally(() => setHistoryLoaded(true));
  }, [orderId, session, apiBase]);

  // Determine current step from real history or mock
  const stepTimestamps: Record<number, string> = {};
  const stepNotes: Record<number, string> = {};
  const stepLocations: Record<number, string> = {};
  let realCurrentStep = -1;

  if (historyLoaded && history.length > 0) {
    history.forEach(entry => {
      const idx = STATUS_TO_STEP[entry.status.toLowerCase()];
      if (idx !== undefined) {
        if (idx > realCurrentStep) realCurrentStep = idx;
        stepTimestamps[idx] = formatTs(entry.timestamp);
        if (entry.note) stepNotes[idx] = entry.note;
        if (entry.location) stepLocations[idx] = entry.location;
      }
    });
  }

  const useMock = !historyLoaded || (historyLoaded && history.length === 0 && !orderId);
  const mockStep = getMockStep(load);
  const currentStep = useMock ? mockStep : Math.max(0, realCurrentStep);
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", animation: "trackFadeUp 0.4s ease" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontFamily: "'Barlow',sans-serif", fontWeight: 700, fontSize: 10, color: "#CC0000", letterSpacing: 4, textTransform: "uppercase", marginBottom: 6 }}>
            ...
            {!useMock && orderId && (
              <span style={{ marginLeft: 12, color: "rgba(0,180,80,0.9)", border: "1px solid rgba(0,180,80,0.3)", borderRadius: 4, padding: "1px 8px", fontSize: 9, fontWeight: 700 }}>
                ● LIVE
              </span>
            )}
          </div>
          <h1 style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: "clamp(26px,4vw,40px)", color: text, textTransform: "uppercase", lineHeight: 1, margin: 0 }}>
            {lang === "ru" ? "ТРЕКИНГ " : "TRACKING "}
            <span style={{ color: "#CC0000" }}>
              #{orderId ? String(orderId).padStart(4, "0") : String(load.id).padStart(4, "0")}
            </span>
          </h1>
        </div>
        ...
        {/* Cancelled banner */}
        {cancelled && (
          <div style={{ background: "rgba(255,77,109,0.08)", border: "1px solid rgba(255,77,109,0.3)", borderRadius: 12, padding: "16px 22px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 28 }}>❌</span>
            <div>
              <div style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: 18, color: "#ff4d6d" }}>
                {lang === "ru" ? "ЗАКАЗ ОТМЕНЁН" : "ORDER CANCELLED"}
              </div>
              <div style={{ fontFamily: "'Barlow',sans-serif", fontSize: 12, color: muted, marginTop: 2 }}>
                {lang === "ru" ? "Этот заказ был отменён" : "This order has been cancelled"}
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ background: isDelivered ? "rgba(0,180,80,0.08)" : "rgba(204,0,0,0.07)", border: `1px solid ${isDelivered ? "rgba(0,180,80,0.3)" : "rgba(204,0,0,0.25)"}`, borderRadius: 12, padding: "18px 22px", marginBottom: 24, display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontFamily: "'Barlow',sans-serif", fontSize: 13, color: muted, marginTop: 3 }}>
                {stepNotes[currentStep] || (lang === "ru" ? currentStepData.descRu : currentStepData.descEn)}
              </div>
              {stepLocations[currentStep] && (
                <div style={{ fontFamily: "'Barlow',sans-serif", fontSize: 12, color: "#CC0000", marginTop: 4 }}>
                  📍 {stepLocations[currentStep]}
                </div>
              )}
            </div>
        ...
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}>
          <RouteProgress load={load} progress={cancelled ? 0 : animProgress} isDark={isDark} lang={lang} />
        </div>
        ...
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: "24px", marginBottom: 20 }}>
          <div style={{ fontFamily: "'Barlow',sans-serif", fontWeight: 700, fontSize: 10, color: "#CC0000", letterSpacing: 3, textTransform: "uppercase", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
            📅 {lang === "ru" ? "ИСТОРИЯ СТАТУСОВ" : "STATUS TIMELINE"}
            {!useMock && (
              <span style={{ fontFamily: "'Barlow',sans-serif", fontSize: 9, fontWeight: 600, color: muted, letterSpacing: 1, textTransform: "none" }}>
                ({history.length} {lang === "ru" ? "записей" : "entries"})
              </span>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {STEPS.map((step, i) => {
              ...
              const isActive  = i === currentStep && !cancelled;
              const isPending = i > currentStep || cancelled;
              const isLast    = i === STEPS.length - 1;
              const ts        = stepTimestamps[i] || (useMock ? (i <= currentStep ? "mock" : "") : "");
              const note      = stepNotes[i];
              const loc       = stepLocations[i];
                ...
                <div key={step.key} style={{ display: "flex", gap: 16, position: "relative" }}>
                  ...
                  <div style={{ flex: 1, paddingBottom: 28 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      <div style={{ fontFamily: "'Barlow',sans-serif", fontWeight: 700, fontSize: 14, color: isDone ? text : isActive ? "#CC0000" : muted }}>
                    ...
                    <div style={{ fontFamily: "'Barlow',sans-serif", fontSize: 12, color: muted, marginTop: 3 }}>
                      {note || (lang === "ru" ? step.descRu : step.descEn)}
                    </div>
                    {loc && (
                      <div style={{ fontFamily: "'Barlow',sans-serif", fontSize: 11, color: isDark ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.5)", marginTop: 3 }}>
                        📍 {loc}
                      </div>
                    )}
                    {ts && ts !== "mock" && (
                    ...
                    {ts === "mock" && (
                      <div style={{ fontFamily: "'Barlow',sans-serif", fontSize: 11, color: "#CC0000", marginTop: 4, opacity: 0.85 }}>
                        🕐 {new Date(Date.now() - (currentStep - i) * 2 * 86400000).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                      </div>
                    )}
                    {isPending && !cancelled && (
