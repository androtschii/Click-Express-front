...
import { FlagCheckered, ShieldCheck, Broadcast, Lightning, MapPin, Crane } from "@phosphor-icons/react";
...
export const TrustStrip: React.FC<TrustStripProps> = ({ theme = 'dark' }) => {
  ...
  const icons = [FlagCheckered, ShieldCheck, Broadcast, Lightning, MapPin, Crane];
      ...
      {icons.map((Icon, i) => {
        ...
        return (
          <div key={label as string} onMouseEnter={() => setHovIdx(i)} onMouseLeave={() => setHovIdx(null)}
            ...
            <Icon size={18} weight="duotone" color={isHov ? "#CC0000" : (isDark ? "rgba(255,255,255,0.52)" : "rgba(13,13,13,0.6)")} style={{ filter: isHov ? "drop-shadow(0 0 4px rgba(204,0,0,0.5))" : "none", transition: "filter 0.15s" }} />
